...
Vector3F operator-(const std::vector<float>& p1, const std::vector<float>& p2) {
    ...
    d.m_values[1] = p1[1] - p2[1];
    d.m_values[2] = p1[2] - p2[2];
    return d;
}

Vector3F operator-(const Vector3F& p1, const Vector3F& p2) {
    Vector3F d;
    d.m_values[0] = p1.m_values[0] - p2.m_values[0];
    d.m_values[1] = p1.m_values[1] - p2.m_values[1];
    d.m_values[2] = p1.m_values[2] - p2.m_values[2];
    return d;
}
...
float operator*(const Vector3F& p1, const Vector3F& p2) {
    return p1.m_values[0] * p2.m_values[0] + p1.m_values[1] * p2.m_values[1] + p1.m_values[2] * p2.m_values[2];
}

Vector3F operator*(const Vector3F& v, float f) {
    Vector3F r;
    r.m_values[0] = v.m_values[0] * f;
    r.m_values[1] = v.m_values[1] * f;
    r.m_values[2] = v.m_values[2] * f;
    return r;
}

std::ostream& operator<<(std::ostream& out, const Vector3F& vec) {
    out << "(" << vec.m_values[0] << ", " << vec.m_values[1] << ", " << vec.m_values[2] << ")";
    return out;
}
